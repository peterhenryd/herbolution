            ...
            for (y, ya) in zip(boundary.y.clone(), adj_boundary.y.clone()) {
                for (z, za) in zip(boundary.z.clone(), adj_boundary.z.clone()) {
                    ...
                    if cullable_faces.contains(face.into()) && adj_cullable_faces.contains(inverse_face.into()) {
                        cube.mesh.remove_faces(face.into());
                ...
                for (z1, z2) in matric.z.clone().zip(other_matric.z.clone()) {
                    ...
                    if !cube
                        ...
                        && !other_cube
                            ...
                            .is_empty()
                    {
                        cube.mesh.remove_faces(Faces::from(shared_face));
                        other_cube
                            .mesh
    ...
    pub fn set(&mut self, position: vec3u5, new_material: Option<PaletteMaterialId>) {
        ...
        if old_material
            ...
            && !new_material
                ...
                .is_empty()
        {
            let missing = self.data[i].mesh.faces().not();
            self.data[i].mesh.set_opaque(Faces::all());
