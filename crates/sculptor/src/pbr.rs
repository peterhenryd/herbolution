...
use gpu::{AtlasTextureCoord, Texture};
...
pub struct PbrTextures {
    ...
    pub(crate) coords: Arc<[AtlasTextureCoord]>,
}
...
fn read_image_or_create_identity(path: Option<&PathBuf>, dimensions: (u32, u32), color: Rgba<u8>) -> DynamicImage {
    if let Some(path) = path {
        ...
        assert_eq!(
            image.dimensions(),
            dimensions,
            "Pbr image texture dimensions do not match diffuse texture dimensions"
        );
