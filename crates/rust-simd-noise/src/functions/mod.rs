...
pub unsafe fn scale_noise<S: Simd>(scale_min: f32, scale_max: f32, min: f32, max: f32, data: &mut [f32]) {
    ...
    if data.len() >= vector_width {
        while i <= data.len() - vector_width {
            let value = (S::Vf32::set1(multiplier) * S::Vf32::load_from_ptr_unaligned(&data[i])) + S::Vf32::set1(offset);
...
pub(crate) unsafe fn get_scaled_noise<const D: NoiseDim, S: Simd, F: Fn(&NoiseType<D>) -> ([f32; D.size()], f32, f32)>(noise_type: &NoiseType<D>, noise_fn: F) ->
[f32; D.size()] {
