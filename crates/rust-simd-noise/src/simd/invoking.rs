...
macro_rules! __simd_generate_base {
    ($(#[$meta:meta])* $vis:vis fn $fn_name:ident <const $D:ident: NoiseDim> ($($arg:ident:$typ:ty),* ) -> $rt:ty $body:block  ) => {
        simd_paste_item! {

            #[inline(always)]
            $vis unsafe fn [<__ $fn_name _generic>] <const $D: NoiseDim, S: $crate::simd::Simd>(args_tuple: ($
            ($typ,)*)) -> $rt {
                let ($($arg,)*) = args_tuple;
                $body
            }
            ...
            $(#[$meta])*
            #[inline(always)]
            $vis fn [<$fn_name _generic>] <const $D: NoiseDim, S: $crate::simd::Simd>($($arg:$typ,)*) -> $rt {
                let args_tuple = ($($arg,)*);
            ...
            impl<const $D: NoiseDim> __SimdRunner<fix_tuple_type!(($($typ),*)), $rt> for [<__ $fn_name _dispatch_struct>] {
                unsafe fn run<S: $crate::simd::Simd>(args_tuple: fix_tuple_type!(($($typ),*))) -> $rt {
                    [<__ $fn_name _generic>]::<$D, S>(args_tuple)
                }
            }
        }
    };
...
macro_rules! simd_runtime_generate {
    ($(#[$meta:meta])* $vis:vis fn $fn_name:ident <const $D:ident: NoiseDim> ($
    ($arg:ident:$typ:ty),* $(,)? ) ->
    $rt:ty $body:block  ) => {
        simd_paste_item! {
            ...
            $(#[$meta])*
            #[inline(always)]
            $vis fn $fn_name <const $D: NoiseDim>($($arg:$typ,)*) -> $rt {
                let args_tuple = ($($arg,)*);
            ...
            $(#[$meta])*
            #[inline(always)]
            $vis fn [<$fn_name _scalar>] <const $D: NoiseDim>($($arg:$typ,)*) -> $rt {
                let args_tuple = ($($arg,)*);
                __run_simd_invoke_scalar::<[<__ $fn_name _dispatch_struct>], fix_tuple_type!(($($typ),*)), $rt>(args_tuple)
            }

            __simd_generate_base!($(#[$meta])* $vis fn $fn_name <const $D: NoiseDim>($($arg:$typ),* ) -> $rt $body);
        }
    };
...
pub fn __run_simd_runtime_decide<S: __SimdRunner<A, R>, A, R>(args: A) -> R {
    ...
    #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
    {
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return unsafe { S::run::<backends::avx2::Avx2>(args) };
        }

        if is_x86_feature_detected!("sse4.1") {
            return unsafe { S::run::<backends::sse41::Sse41>(args) };
        }

        if is_x86_feature_detected!("sse2") {
            return unsafe { S::run::<backends::sse2::Sse2>(args) };
        }
    }

    #[cfg(target_arch = "aarch64")]
    if is_aarch64_feature_detected!("neon") {
        return unsafe { S::run::<backends::neon::Neon>(args) };
    }

    #[cfg(target_arch = "wasm32")]
    {
        return unsafe { S::run::<backends::wasm32::Wasm>(args) };
    }

    unsafe { S::run::<backends::scalar::Scalar>(args) }
}
...
pub fn __run_simd_compiletime_select<S: __SimdRunner<A, R>, A, R>(args: A) -> R {
    ...
    return unsafe { S::run::<backends::scalar::Scalar>(args) };
}
