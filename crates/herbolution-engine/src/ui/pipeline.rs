...
use wgpu::{
    ...
    BindGroupLayoutEntry, BindingType, Buffer, BufferBindingType, BufferUsages, Queue, RenderPipeline,
    ShaderStages,
};
use crate::gpu::Gpu;
...
impl UiPipeline {
    pub fn create(gpu: &Gpu, surface: &Surface<'_>, projection: &Projection) -> Self {
    ...
    pub fn update_uniforms(&self, projection: &Projection) {
        self.queue
            .write_buffer(&self.uniform_buffer, 0, projection.as_ref());
    }
}
