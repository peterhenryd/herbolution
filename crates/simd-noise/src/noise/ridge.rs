...
use crate::functions::ridge_32::{ridge_1d, ridge_2d, ridge_3d, ridge_4d};
...
#[derive(Copy, Clone)]
pub struct RidgeNoise<const D: NoiseDim> {
...
impl<const D: NoiseDim> DimNoise<D> for RidgeNoise<D> {
    fn dim(&self) -> NoiseTransform<D> {
        self.dim
    }
}

impl<const D: NoiseDim> From<NoiseTransform<D>> for RidgeNoise<D> {
...
impl<const D: NoiseDim> Noise<D> for RidgeNoise<D> {
...
impl<const D: NoiseDim> OctaveNoise for RidgeNoise<D> {
...
impl<const D: NoiseDim, S: Simd> Sample32<D, S> for RidgeNoise<D> {
    #[inline(always)]
    fn sample_1d(&self, x: S::F32) -> S::F32 {
        ridge_1d::<S>(x, S::F32::set1(self.lacunarity), S::F32::set1(self.gain), self.octaves, self.dim.seed)
    }

    #[inline(always)]
    fn sample_2d(&self, x: S::F32, y: S::F32) -> S::F32 {
        ridge_2d::<S>(x, y, S::F32::set1(self.lacunarity), S::F32::set1(self.gain), self.octaves, self.dim.seed)
    }

    #[inline(always)]
    fn sample_3d(&self, x: S::F32, y: S::F32, z: S::F32) -> S::F32 {
        ridge_3d::<S>(x, y, z, S::F32::set1(self.lacunarity), S::F32::set1(self.gain), self.octaves, self.dim.seed)
    }

    #[inline(always)]
    fn sample_4d(&self, x: S::F32, y: S::F32, z: S::F32, w: S::F32) -> S::F32 {
        ridge_4d::<S>(x, y, z, w, S::F32::set1(self.lacunarity), S::F32::set1(self.gain), self.octaves, self.dim.seed)
    }
}

/*
impl<const D: NoiseDim, S: Simd> Sample<S, D> for RidgeNoise<D> {
    #[inline(always)]
