...
impl<'a, T> Traverse<'a, T> {
    ...
    pub fn iter(&self) -> TraverseIter<'_, T> {
        TraverseIter {
            tree: self.tree,
            node: Some(self.tree.nodes[0]),
            index: 0,
        }
    }
}
...
impl<'a, T> Iterator for TraverseIter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.node? {
            Node::Leaf { index } => {
                self.node = self.tree.nodes.get(index).copied();
                Some(unsafe { self.tree.values[index].assume_init_ref() })
            }
            Node::Branch { start_index } => {
                if self.index < 8 {
                    let node_index = start_index + self.index;
                    self.index += 1;
                    self.node = Some(self.tree.nodes[node_index]);
                    if let Node::Leaf { index } = self.node? {
                        Some(unsafe { self.tree.values[index].assume_init_ref() })
                    } else {
                        None
                    }
