...
use image::{DynamicImage, EncodableLayout, GenericImageView, GrayImage, RgbaImage};
use image_atlas::{AtlasDescriptor, AtlasEntry, AtlasEntryMipOption, AtlasError, AtlasMipOption};
use lib::size::size2u;
...
impl Texture {
    ...
    pub fn from_data<T>(gpu: &Handle, size: impl Into<size2u>, format: TextureFormat, data: impl AsRef<[T]>) -> Self
    where
        [T]: EncodableLayout,
    {
        let size = size.into();
        ...
        gpu.queue().write_texture(
            ...
            TexelCopyBufferLayout {
                offset: 0,
                bytes_per_row: Some(size.width * 4 * size_of::<T>() as u32),
    ...
    pub fn from_image(gpu: &Handle, format: TextureFormat, image: DynamicImage) -> Self {
        Self::from_data(gpu, image.dimensions(), format, image.as_bytes())
    }

    pub fn from_rgba(gpu: &Handle, image: RgbaImage) -> Self {
        Self::from_image(gpu, TextureFormat::Rgba8UnormSrgb, image.into())
    }

    pub fn from_gray(gpu: &Handle, image: GrayImage) -> Self {
        Self::from_image(gpu, TextureFormat::R8Unorm, image.into())
    }
