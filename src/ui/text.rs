...
impl UiText {
    pub fn create(gpu: &Gpu, surface: &Surface) -> Self {
        ...
        let mut atlas = TextAtlas::new(&gpu.device, &gpu.queue, &cache, surface.get_texture_format());
        let renderer = TextRenderer::new(&mut atlas, &gpu.device, MultisampleState::default(), None);
    ...
    pub fn resize(&mut self, PhysicalSize { width, height }: PhysicalSize<u32>) {
        self.viewport.update(&self.gpu.queue, Resolution { width, height });
    }
    ...
    pub fn prepare(&mut self) {
        ...
        let text_areas = self.displays.0.iter()
            .map(|x| TextArea {
                buffer: &x.buffer,
                left: x.pos[0],
                top: x.pos[1],
    ...
    pub fn build_text(&mut self, font_size: f32) -> TextBuilder {
        let buffer = Buffer::new(&mut self.font_system, Metrics::new(font_size, font_size * 1.5));
    ...
    pub fn render(&self, render_pass: &mut RenderPass) {
        self.renderer.render(&self.atlas, &self.viewport, render_pass).expect("Failed to render text");
    }
...
impl<'a> TextBuilder<'a> {
    ...
    pub fn finish(mut self) -> TextHandle {
        self.buffer.set_text(self.font_system, &self.text, self.attrs, self.shaping);
...
impl TextDisplay {
    pub fn edit(&mut self, font_system: &mut FontSystem, f: impl FnOnce(TextEditor) -> TextEditor) {
        ...
        self.buffer.set_text(font_system, &self.text, self.attrs, self.shaping);
    }
}
